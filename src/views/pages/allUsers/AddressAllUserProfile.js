...
function AddressAllUserProfile() {
/////////////////////////////
  ...
  const resultUsers = users.map((element, index)=>{
      return(
          <>
          <option key={index}> {element.id_user} -{element.name}</option>
          </>
      )
  });
  ...
  const newidUser = user.split("-");
      ...
      .then((response)=>{
          if(response.ok){
              ...
              localStorage.setItem('adrress_user',  endereco);
      ...
      <Container fluid>
        <Row>
          <Col md="12">
            <Card>
              <Card.Header>
                <Card.Title as="h4">Endereço Usuário</Card.Title>
              </Card.Header>
              <Card.Body>
...
export default AddressAllUserProfile;
